...
const ResponsesTab = () => {
  ...
  const fetchResponses = async (page = 1) => {
    try {
      setLoading(true);
      const data = await adventurerService.getResults(page, 10);
  ...
  const handleDeleteResponse = async () => {
    ...
    try {
      await adventurerService.deleteSubmission(selectedResponse._id);
  ...
  return (
    <div className="space-y-4">
      <Card>
        <CardHeader>
          <CardTitle>
            Soumissions Profil d'Aventurier ({totalResults} au total)
          </CardTitle>
        </CardHeader>
        <CardContent>
          {responses.length === 0 ? (
