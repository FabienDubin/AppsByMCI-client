...
const ConfigTab = () => {
  ...
  const fetchConfig = async () => {
    try {
      const response = await adventurerService.getConfig();
  ...
  const handleSubmit = async () => {
    ...
    // console.log(req);
    try {
      const response = await adventurerService.updateConfig(req);
  ...
  return (
    <div className="w-full min-h-screen">
      <h1 className="text-center text-lg font-medium my-4">
        Ici tu peux configurer les questions du quiz d'aventurier et optimiser
        le prompt pour la génération de l'avatar.
      </h1>
      {isLoading && (
      ...
      {questions.length > 0 && (
        <Accordion type="single" collapsible className="w-full">
          ...
          <AccordionItem value="code">
            ...
            <AccordionContent>
              Ce code de sécurité permet de vérifier que le quiz d'aventurier
              est lancé par un utilisateur autorisé. Il doit être fourni lors du
              début de la session.
              <Input
                value={code}
          ...
          <AccordionItem value="prompt">
            <AccordionTrigger>Prompt de génération</AccordionTrigger>
            <AccordionContent>
              Tu peux éditer le prompt pour la génération de l'avatar
              d'aventurier mais assure toi de garder les variables qui
              permettent la personnalisation <code>{"{{name}}"}</code>,{" "}
              <code>{"{{gender}}"}</code>, <code>{"{{answers}}"}</code>.
              <Textarea
                className="h-48"
                value={promptTemplate}
                onChange={(e) => setPromptTemplate(e.target.value)}
                placeholder="Prompt de génération d'avatar d'aventurier"
              />
            </AccordionContent>
          </AccordionItem>
        </Accordion>
